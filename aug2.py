...
train_dir = os.path.join("data_augmentation", "train")
val_dir = os.path.join("data_augmentation", "val")
data_transforms = transforms.Compose([transforms.ColorJitter(), transforms.ToTensor()])
...
for i, (img, label) in enumerate(train_dataset):
	...
	save_image(img, f'{train_dir}/{label}/{i}.png')
	prev_label = label

for i, (img, label) in enumerate(val_dataset):
	if i == 0 or label != prev_label:
		os.mkdir(f'{val_dir}/{label}')
	save_image(img, f'{val_dir}/{label}/{i}.png')
	prev_label = label
